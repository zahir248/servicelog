...
class Vehicle extends Component {
  ...
  handleDelete = async () => {
    ...
    try {
      const res = await axios.delete(`http://localhost:8000/api/vehicle/${vehicleToDelete}`, {
        headers: {
          Authorization: `Bearer ${userToken}`,
        },
      });
      ...
      if (res.data.status === 200) {
        this.setState({
          ...
          vehicles: this.state.vehicles.filter((vehicle) => vehicle.id !== vehicleToDelete), // Remove the deleted vehicle
  ...
  render() {
    ...
    const { vehicles, loading, successMessage, showModal, showLogoutModal } = this.state;
  
    let vehicleHTML;
  
    // Check if vehicles are loading or if no vehicles are fetched
    if (loading) {
      vehicleHTML = <h2>Loading...</h2>;
          ...
          <tbody>
            {vehicles.map((item) => (
              <tr key={item.id}>
                ...
                <td>
                  <Link to={`/service-history/${item.id}`} className="btn btn-warning btn-sm" title="View Service History">
                    <i className="bi bi-tools"></i>
                  </Link>
                  <Link to={`/edit-vehicle/${item.id}`} className="btn btn-success btn-sm ml-2" title="Edit Vehicle">
                    <i className="bi bi-pencil"></i>
                  </Link>
                  <button onClick={() => this.openModal(item.id)} className="btn btn-danger btn-sm ml-2" title="Delete Vehicle">
      ...
      <div className="container-fluid custom-container">
        <div className="row justify-content-center">
          <main role="main" className="col-12 px-4">
            <h4 className="text-center">List of Vehicles</h4>
        ...
        <Link to="/add-vehicle" className="btn btn-primary btn-floating" title="Add Vehicle">
          <i className="bi bi-plus"></i>
        </Link>

        {/* Floating Logout Button */}
        <button onClick={this.openLogoutModal} className="btn btn-danger btn-floating logout-btn" title="Logout">
            ...
            <div className="modal-dialog">
              <div className="modal-content">
                ...
                <div className="modal-body">
                  Are you sure you want to delete this vehicle? Please note that this action will also delete all associated service records.
                </div>
        ...
        {showLogoutModal && (
          <div className="modal show" style={{ display: "block" }} tabIndex="-1" aria-labelledby="logoutModalLabel" aria-hidden="true">
            <div className="modal-dialog">
              <div className="modal-content">
                <div className="modal-header">
                  <h5 className="modal-title" id="logoutModalLabel">Confirm Logout</h5>
                  <button type="button" className="btn-close" onClick={this.closeLogoutModal}></button>
                </div>
                <div className="modal-body">
                  Are you sure you want to log out?
                </div>
                <div className="modal-footer">
                  <button type="button" className="btn btn-secondary" onClick={this.closeLogoutModal}>Cancel</button>
